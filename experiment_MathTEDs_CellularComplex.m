...
vertex_frequency = zeros(size(MathTEDs,2));
vertices = [];
vertex_labels = [];
for i = 1:size(MathTEDs,1)
    for j = 1:size(MathTEDs,2)
        if (MathTEDs(i,j) == 1)
...
vertex_label_map = zeros(size(MathTEDs,2));
...
options.filename = 'FilteredCellularMathTEDBarcodes';
