...
vertex_frequency = zeros(size(DesignTEDs,2));
vertices = [];
vertex_labels = [];
for i = 1:size(DesignTEDs,1)
    for j = 1:size(DesignTEDs,2)
        if (DesignTEDs(i,j) == 1)
...
vertex_label_map = zeros(size(DesignTEDs,2));
...
options.filename = 'FilteredCellularDesignTEDBarcodes';
