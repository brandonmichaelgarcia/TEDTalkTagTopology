...
vertex_frequency = zeros(size(allTEDs,2));
vertices = [];
vertex_labels = [];
...
vertex_label_map = zeros(size(allTEDs,2));
for i = 1:size(vertices,1)
    filtration_value = timeline( vertex_labels(i) );
...
for i = 1:size(allTEDs,1)
    ...
    if (size(edgesInCell, 1) > 1)
        ...
        TwoCell_filtration_values = [TwoCell_filtration_values; timeline(i)];
    else
        count = count + 1;
    end
end
...
persistence = api.Plex4.getModularCellularAlgorithm(max_dimension + 1,3);
intervals = persistence.computeIntervals(stream);
Betti_numbers = intervals.getBettiNumbers()


options.filename = 'TimeFilteredAllTEDBarcodes';
